...
const StaffSchedule = () => {
  ...
  const { 
    ...
    () => staffService.getFullStaffSchedule(user?.id),
    {
 ...
 const schedule = useMemo(() => {
  const grouped = {}
  const rawAppointments = scheduleData?.data || []

  rawAppointments.forEach(appt => {
    const date = appt.date // ✅ Make sure this matches your object (not appointment_date)
    if (date) {
...
console.log('🔎 Days in weekly view:', weekDays.map(d => d.toISOString().split('T')[0]))
